...
class OrganizationMigrationTest {
    @Test fun versionThreeDataAndHistorySurviveMigration() = runTest {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val name = "organization-migration-" + java.util.UUID.randomUUID() + ".db"
        val json = InstrumentationRegistry.getInstrumentation().context.assets.open("it.notes.ecosystem.data.NotesDatabase/3.json").bufferedReader().use { JSONObject(it.readText()) }
        val helper = FrameworkSQLiteOpenHelperFactory().create(SupportSQLiteOpenHelper.Configuration.builder(context).name(name)
            .callback(object : SupportSQLiteOpenHelper.Callback(3) {
                override fun onCreate(db: SupportSQLiteDatabase) {
                    ...
                    db.execSQL("INSERT INTO note_revisions (revisionId,noteId,title,body,collectionId,savedAt) VALUES ('rev','n','Storico','Da conservare',NULL,0)")
        ...
        try {
            ...
            val database = Room.databaseBuilder(context, NotesDatabase::class.java, name).addMigrations(NotesDatabase.MIGRATION_3_4, NotesDatabase.MIGRATION_4_5, NotesDatabase.MIGRATION_5_6, NotesDatabase.MIGRATION_6_7).build()
            try {
                ...
                assertFalse(repo.get("n")!!.pinned); assertFalse(repo.get("n")!!.archived)
                assertEquals("Da conservare", repo.history("n").single().body)
                repo.setPinned("n",true); repo.setArchived("n",true)
                assertTrue(repo.get("n")!!.pinned); assertTrue(repo.get("n")!!.archived)
