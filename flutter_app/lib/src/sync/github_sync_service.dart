...
class GitHubSyncService {
  ...
  Future<void> _saveRecords(
    ...
    await temp.rename(file.path);
      ...
      throw const FormatException('Percorso GitHub non valido.');
    }
  }

  Future<RemoteBlob?> readFile(
    String relativePath,
    String ref, {
    int limit = 1024 * 1024,
  }) async {
    _validateRelativePath(relativePath);
    final path = _path('${config.folder}/$relativePath');
    String text;
    try {
      text = await _request(
        'GET',
    ...
    if (decoded is! Map ||
      ...
      throw const FormatException('File GitHub non valido.');
    }
...
class GitHubSyncService {
  ...
  Future<void> _saveRecords(
    ...
    await temp.rename(file.path);
      ...
      throw const FormatException('SHA GitHub non valido.');
    }
    final raw = decoded['content']?.toString() ?? '';
    final bytes = Uint8List.fromList(
    ...
    if (bytes.length > limit) {
      throw const FormatException('File GitHub oltre il limite consentito.');
    }
    return RemoteBlob(bytes: bytes, sha: sha);
  }
  ...
  Future<String> writeFile(
    ...
    if (bytes.isEmpty || bytes.length > limit) {
      throw const FormatException('File GitHub vuoto o troppo grande.');
    }
    final path = _path('${config.folder}/$relativePath');
    final decoded = jsonDecode(
      await _request(
...
class GitHubSyncService {
  ...
  Future<void> _saveRecords(
    ...
    await temp.rename(file.path);
      ...
      throw const FormatException('SHA upload GitHub non valido.');
    }
    return sha;
  }
...
class GitHubSyncService {
  ...
  Future<Map<String, RemoteAsset>> listAssets(String ref) =>
      _assetIndex(ref);
