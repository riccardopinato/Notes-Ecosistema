...
class CaptureEditorTest {
    ...
    private fun open(seed: CaptureSeed): EditorViewModel {
        lateinit var model: EditorViewModel
        instrument.runOnMainSync {
            model = ViewModelProvider(store, object : ViewModelProvider.Factory {
                @Suppress("UNCHECKED_CAST")
                override fun <T : ViewModel> create(modelClass: Class<T>): T =
                    EditorViewModel(repo, SavedStateHandle(mapOf("id" to "capture-test", "new" to true)), seed) as T
    ...
    @Test fun sharedTextIsOnlyADraftUntilSave() = runBlocking {
        val model = open(CaptureSeed("Titolo", "Testo"))
        assertTrue(repo.snapshot().notes.isEmpty())
        assertEquals("Testo", repo.getDraft("capture-test")?.body)
        instrument.runOnMainSync { assertTrue(model.dirty) }
        var saved = false
        instrument.runOnMainSync { model.save { saved = true } }
        waitUntil { saved }
        assertEquals("Testo", repo.get("capture-test")?.body)
        assertNull(repo.getDraft("capture-test"))
    }
    @Test fun reconstructedEditorKeepsEditedDraftInsteadOfOriginalShare() = runBlocking {
        val model = open(CaptureSeed("", "Originale"))
        instrument.runOnMainSync { model.editBody("Modificato") }
        assertEquals("Modificato", repo.getDraft("capture-test")?.body)
        instrument.runOnMainSync { store.clear(); store = ViewModelStore() }
        val restored = open(CaptureSeed("", "Originale"))
        instrument.runOnMainSync { assertEquals("Modificato", restored.body) }
        assertEquals(1, repo.snapshot().drafts.size)
    }
    @Test fun discardDoesNotCreateASavedNote() = runBlocking {
        val model = open(CaptureSeed("", "Da scartare"))
        var discarded = false
        instrument.runOnMainSync { model.discard { discarded = true } }
        waitUntil { discarded }
        assertTrue(repo.snapshot().notes.isEmpty())
        assertNull(repo.getDraft("capture-test"))
    }
